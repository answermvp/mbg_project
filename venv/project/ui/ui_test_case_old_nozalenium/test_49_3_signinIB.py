...
class SigninIB_test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # 客户经理开户链接注册IB
        # cls.page.get('https://awstau-www.aetos.me/30300002-IB')
        # new
        cls.page.get('https://awst49-trust.aetos.me/30300002-IB')
    ...
    def test_1_signinIB_1(self, email, password, password2, phone):
        """测试注册IB第1步"""
        self.signinIB_1(email, password, password2, phone)

    # @data(
    #     ['test_answer_205@email.com', 'test_answer_205@email.com', '999205', 'testsurname', 'testlastname', '中文名', '999205', '1111-11-11', '深圳', '宝安区', '1','baoanqu', '测试', '维拓利亚', '其他']
    # )
    ...
    def test_2_signinIB_2(self, expect_username, expect_email, expect_phone, surName, lastName, nickName, papers_num, birthday, city, address, addressDate, addressEn, position, employerName, incomeComment):
        ...
        step2_message = self.signinIB_2(surName, lastName, nickName, papers_num, birthday, city, address, addressDate, addressEn, position, employerName, incomeComment)
        # 断言自动载入的信息是否符合预期
        self.assertEqual(step2_message[0], expect_username)
        self.assertEqual(step2_message[1], expect_email)
        self.assertEqual(step2_message[2], expect_phone)

    # @data(
    #     ['恭喜您注册成功！', 'test_answer_205@email.com.']
    # )
